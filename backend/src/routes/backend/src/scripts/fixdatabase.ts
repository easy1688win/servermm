...
async function insertGameOperationalPermission() {
  try {
    logSection('🔐 INSERTING GAME OPERATIONAL PERMISSION');
    
    // 检查权限是否已存在
    const existingPermission: any[] = await sequelize.query(
      `SELECT id FROM Permissions WHERE slug = 'action:game_operational'`,
      { type: QueryTypes.SELECT }
    );

    if (existingPermission.length > 0) {
      logToFile('⚠️  Game operational permission already exists, skipping insertion.');
      return;
    }

    // 插入新权限
    await sequelize.query(
      `INSERT INTO Permissions (slug, description, createdAt, updatedAt) 
       VALUES ('action:game_operational', 'Game Operational Actions - Edit, adjust balance, and delete games', NOW(), NOW())`,
      { type: QueryTypes.INSERT }
    );

    logToFile('✅ Game operational permission inserted successfully.', 'SUCCESS');

    // 获取插入的权限ID
    const newPermission: any[] = await sequelize.query(
      `SELECT id FROM Permissions WHERE slug = 'action:game_operational'`,
      { type: QueryTypes.SELECT }
    );
    ...
    if (newPermission.length > 0) {
      ...
      logToFile(`📋 New permission ID: ${permissionId}`);

      // 为超级管理员角色添加此权限（如果存在）
      const superAdminRole: any[] = await sequelize.query(
        `SELECT id FROM Roles WHERE name IN ('Super Admin', 'SuperAdmin', 'super_admin') LIMIT 1`,
        { type: QueryTypes.SELECT }
      );
      ...
      if (superAdminRole.length > 0) {
        ...
        // 检查角色权限是否已存在
        const existingRolePermission: any[] = await sequelize.query(
          `SELECT roleId, permissionId FROM Role_Permissions WHERE roleId = ? AND permissionId = ?`,
          { 
        ...
        if (existingRolePermission.length === 0) {
          ...
          logToFile(`✅ Game operational permission assigned to Super Admin role.`, 'SUCCESS');
        } else {
          logToFile('⚠️  Game operational permission already assigned to Super Admin role.');
        }
      } else {
        logToFile('⚠️  Super Admin role not found, permission not assigned to any role.');
      }
    }

  } catch (error) {
    logToFile(`❌ Error inserting game operational permission: ${error}`, 'ERROR');
    throw error;
  }
}
...
async function initDb() {
  ...
  try {
    ...
    // 插入游戏操作权限
    await insertGameOperationalPermission();
