...
async function initDb() {
  try {
    ...
    // One-time migration: rename bank_accounts.account_number_full -> account_number if needed
    const [fullCol]: any = await sequelize.query("SHOW COLUMNS FROM `bank_accounts` LIKE 'account_number_full';");
    const [numberCol]: any = await sequelize.query("SHOW COLUMNS FROM `bank_accounts` LIKE 'account_number';");
    if (Array.isArray(fullCol) && fullCol.length > 0 && (!Array.isArray(numberCol) || numberCol.length === 0)) {
      console.log('Migrating bank_accounts.account_number_full -> account_number ...');
      await sequelize.query("ALTER TABLE `bank_accounts` CHANGE `account_number_full` `account_number` VARCHAR(255);");
    ...
    const adminPassword = 'admin'; // Should be changed
